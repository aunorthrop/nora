// Nora voice brain: STT -> intent router -> TTS
// Netlify Functions / Node 18; no external deps.
...
const OPENAI_TTS_VOICE = process.env.OPENAI_TTS_VOICE || "shimmer"; // brighter voice

// windows
const UPDATES_TTL_HOURS   = Number(process.env.UPDATES_TTL_HOURS   || 168); // 7 days
const BRIEF_WINDOW_HOURS  = Number(process.env.BRIEF_WINDOW_HOURS  || 24);  // "what's new?" window

// simple in-mem store per instance
...
export const handler = async (event)=>{
  ...
  try{
    ...
    const role = String(body.role||"employee");           // "admin" | "employee"
    const expectRole = !!body.expectRole;                 // first-run yes/no
    ...
    if(!raw) return say(team(businessId),"I didn’t catch that. Try again a little closer to the mic.");

    const state = team(businessId);
    const lower = raw.toLowerCase();

    // ---- first-run: decide role (yes/no/admin/employee) ----
    if(expectRole){
      const isYes = /\b(yes|yeah|yep|i am|i'm|admin)\b/.test(lower);
      const isNo  = /\b(no|nope|not|employee|staff)\b/.test(lower);
      if(isYes && !isNo) return say(state,"Great. This device is set as admin. I’ll remember what you tell me for the team.",{control:{role:"admin"}});
      if(isNo  && !isYes) return say(state,"Got it. This device is an employee device.",{control:{role:"employee"}});
      return say(state,"Sorry, was that yes or no? Say yes if you’re the admin, otherwise say no.",{control:{askRoleAgain:true}});
    }
    ...
    // ======================
    // ADMIN: everything you speak becomes knowledge
    // ======================
    if(role==="admin"){
      // delete/forget from updates or long-term
      if(/^(delete|remove|forget)\b/.test(lower)){
        ...
        if(uIdx>=0){ state.updates.splice(state.updates.length-1-uIdx,1); return say(state,"Deleted that update."); }
        const lIdx = [...state.longterm].reverse().findIndex(u=>u.text.toLowerCase().includes(tail.toLowerCase()));
        if(lIdx>=0){ state.longterm.splice(state.longterm.length-1-lIdx,1); return say(state,"Deleted that long-term note."); }
        return say(state,"I didn’t find a matching note to delete.");
      }

      // classify: long-term vs update
      const isLongTerm = /(\bpermanent\b|\balways\b|\bpolicy\b|\bhandbook\b|\bprocedure\b|\baddress\b|\bphone\b|\bhours\b|\bsafety\b|\bmenu\b|\bforever\b|\bremember\b|\bkeep\b|\bstore\b|\bsave\b)/i.test(raw);
      if(isLongTerm){
        state.longterm.push({text:raw.replace(/^(remember|save|store|keep)\b[:\-]?\s*/i,"").trim(), ts:now()});
        prune(state);
...
function whatsNewMsg(state){
  ...
  return recent.length? "Here’s the latest. "+bullets(recent.slice(-8)) : "No new updates right now.";
}
...
async function transcribeRobust(b64, mime){
  ...
  const attempts=[
    {mime:norm(mime),ext:extFor(mime)},
...
// speak + remember last line
async function say(state, text, extra){
  state.lastSay = text;
  const audio = await tts(text);
  return j({audio, ...(extra||{})});
}
